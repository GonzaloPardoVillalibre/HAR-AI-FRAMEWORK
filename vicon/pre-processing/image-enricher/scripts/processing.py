...
def fold_position_image(input_file: str, output_file: str, image_size, sensors_number, column_names: list):
    ...
    # ADD HERE FFT CALCULATION
    final_df.to_csv(output_file)

def fold_orientation_image(input_file: str, output_file: str, image_size, sensors_number, column_names: list):
    df = pd.read_fwf(input_file, header=None)
    df = df[0].str.split(',', expand=True)
    df = df.iloc[1:]
    df = df.drop(df.columns[[0, 1]], axis=1)
...
def fold_images():
    ...
    for orientation_folder_name in dt_cfg["movements"]["list"]:
      ...
      for file in files:
          fold_orientation_image(input_folder+file, output_folder+file, image_size, sensors_number, column_names)
