...
def load_model(input_rows:int, input_columns:int, channels:int, movements_number:int):
    ...
    model = tf.keras.Sequential([
        ...
        layers.Conv2D(64, activation='relu', kernel_size=3,padding='same',kernel_regularizer=tf.keras.regularizers.L2(0.01)),
        layers.MaxPooling2D(pool_size=2, strides=2, padding='same'),
        layers.BatchNormalization(axis=1),
        layers.Conv2D(128, activation='relu', kernel_size=3,padding='same',kernel_regularizer=tf.keras.regularizers.L2(0.01)),
        layers.MaxPooling2D(pool_size=2, strides=2, padding='same'),
        layers.BatchNormalization(axis=1),
        layers.Conv2D(256, activation='relu', kernel_size=3,padding='same',kernel_regularizer=tf.keras.regularizers.L2(0.01)),
        layers.MaxPooling2D(pool_size=2, strides=2, padding='same'),
        layers.BatchNormalization(axis=1),
        #layers.Flatten(),
